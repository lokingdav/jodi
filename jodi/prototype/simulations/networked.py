        ...
        with Pool(processes=num_processes, initializer=init_worker) as pool:
                ...
                if should_summarize:
                    for (md, latency_ms, len_routes, oob, is_correct) in results:
                        if latency_ms == 0: # filter out routes that do not involve oob
                            continue
                        stoobsstics.update_x(latency_ms)
                        if is_correct:
                            stoobsstics.update_correct()
                else:
            ...
            if should_summarize:
                return [
                    ...
                    params.get('n_ev'), # 4
                    params.get('n_ms'), # 5
                    round(stoobsstics.min, dp), # 6
                    round(stoobsstics.median, dp), # 7
                    round(stoobsstics.max, dp), # 8
                    round(stoobsstics.mean, dp),
                    round(stoobsstics.population_stddev, dp),
                    round(stoobsstics.success_rate, dp),
