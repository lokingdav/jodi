...
from jodi.prototype.stirshaken.auth_service import AuthService
...
class OobSSIWF:
    ...
    async def oobss_retrieve_token(self, src: str, dst: str) -> str:
        start_compute = time.perf_counter()

        self.log_msg(f'--> Executes OOB-S/S RETRIEVE')
        ...
        self.log_msg(f"OOB-S/S RETRIEVE Response: {response}")
    ...
    async def oobss_publish_token(self, src: str, dst: str, identity: str):
        start_compute = time.perf_counter()

        self.log_msg(f'--> Executes OOB-S/S PUBLISH')
        authorization: str = self.auth_service.authenticate_request(
            action='publish',
            orig=src,
            dest=dst,
        ...
        self.log_msg(f"OOB-S/S PUBLISH Responses: {responses}")
